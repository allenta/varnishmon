...
type APIWorkerErrorLogAdaptor struct {
	log *config.Logger
}

func (awla *APIWorkerErrorLogAdaptor) Write(p []byte) (int, error) {
	awla.log.Error().Msg(string(p))
	return len(p), nil
}
...
func (aw *APIWorker) init() {
	aw.server = &http.Server{
		...
		ErrorLog: log.New(&APIWorkerErrorLogAdaptor{log: aw.app.Cfg().Log()}, "", 0),
	}
}
...
func (aw *APIWorker) run() {
	...
	ln, err := lc.Listen(aw.worker.ctx, "tcp", aw.server.Addr)
	if err != nil {
	...
	if aw.app.Cfg().APITLSCertfile() != "" && aw.app.Cfg().APITLSKeyfile() != "" {
		if err := aw.server.ServeTLS(ln, aw.app.Cfg().APITLSCertfile(), aw.app.Cfg().APITLSKeyfile()); err != nil &&
		...
		if err := aw.server.Serve(ln); err != nil && !errors.Is(err, http.ErrServerClosed) {
			aw.app.Cfg().Log().Fatal().
				Err(err).
				Str("address", aw.server.Addr).
				Msgf("API worker '%v' failed to serve!", aw)
